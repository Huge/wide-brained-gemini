...
function renderPresetButtons(presets, widthMin, widthMax, currentSetting) {
    ...
    (presets || currentPresets).forEach((preset, i) => {
        ...
        valSpan.className = 'btn-value';
        valSpan.textContent = formatWidthValue(preset);

        btn.append(iconSpan, textSpan, valSpan);
...
function renderPresetButtonsEditMode(presets) {
    ...
    (presets || currentPresets).forEach((preset, i) => {
        ...
        labelSpan.className = 'preset-edit-label';
        labelSpan.title = name;
        labelSpan.textContent = name;

        const input = document.createElement('input');
        input.type = 'number';
        input.className = 'preset-edit-width';
        ...
        unitSpan.className = 'preset-edit-unit';
        unitSpan.dataset.index = i;
        unitSpan.textContent = getUnitLabel(currentWidthSetting.unit);

        row.append(labelSpan, input, unitSpan);
...
function renderManagePresetsRows(presets) {
    ...
    (presets || currentPresets).forEach((preset, i) => {
        ...
        widthInput.className = 'manage-width';
        widthInput.value = preset.value;
        widthInput.min = range.min;
        widthInput.max = range.max;
        widthInput.step = range.step;
        widthInput.dataset.index = i;

        const unitSpan = document.createElement('span');
        unitSpan.className = 'manage-unit';
        unitSpan.dataset.index = i;
        unitSpan.textContent = getUnitLabel(currentWidthSetting.unit);

        const resetBtn = document.createElement('button');
        resetBtn.type = 'button';
        resetBtn.className = 'secondary-btn reset-preset-btn';
        resetBtn.dataset.index = i;
        resetBtn.textContent = t('resetToDefault', '重置为默认');

        row.append(nameInput, widthInput, unitSpan, resetBtn);
